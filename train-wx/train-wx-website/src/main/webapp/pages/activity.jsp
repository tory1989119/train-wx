...
<body>
	<div class="page-group">
		<div class="page page-current">
			...
			<div id="wrapper">
				<div id="scroller">
					<div class="content native-scroll" id="activitys"></div>
					<div id=pullUp>
	...
	function loadData()
	{
		...
		$.ajax({
			url: "<%=request.getContextPath()%>/activity/queryActivity.do",
			datatype: 'json',
			type: "post",
			data: {
				begin:lastpage,
			...
			success: function (data) {
				if (data.flag == '1' && data.errorCode == '10000') {
					if(data.content.length > 0){
						...
						$.each(data.content,function(i,activitys){
							
							var html = '';
							html = html + '<div class="imgbox">';
							html = html + '<img src="' + activitys.img + '">';
							html = html + '</div>';
							...
							html = html + '<div class="item-title-row"><div class="item-title icon-vip">' + activitys.organizationName + '</div>';
							html = html + '</div>';
							html = html + '<div class="item-subtitle">';
							html = html + '<div class="sub_txt s_font">';
							html = html + '<div>活动名称：<span>' + activitys.name + '</span></div>';
							html = html + '<div>活动时间：<span>' + activitys.activityTime + '</span></div>';
							html = html + '<div>活动地址：<span>' + activitys.address + '</span></div>';
							html = html + '<div>参与对象：<span>' + activitys.age + '</span></div>';
							
							if(activitys.chargeType == '0'){
								html = html + '<div>活动费用：<span>免费</span></div>';
							}else{
								html = html + '<div>活动费用：<span>自费</span></div>';
							}
							
							html = html + '</div></div></div><div class="item-media2">';
							html = html + '<a href="tel:' + activitys.telephone + '" class="button button-orange">报名咨询</a></div></div></li></ul></div>';
